    ...
    if (!visualDesign || visualDesign.error || !visualDesign.variations) {
      ...
      try {
        ...
        const visionResult = await visionModel.generateContent([
          "You are an expert architect. Analyze this 2D floor plan image and identify the front elevation structure exactly. Write a highly detailed, strict text prompt to generate an exterior 3D elevation image of this house. If the plan shows a single floor, you MUST start the prompt with: 'STRICTLY SINGLE-STORY (1 floor ONLY) ultra-realistic modern Indian house front elevation. STYLE: Clean off-white/cream exterior walls with light grey accent bands, flat roof.' Then, EXACTLY list the structural elements on the left, center, and right of the front view based ONLY on the drawing. If there is a portico/parking, describe it. If there is a staircase in the front, describe it. Do NOT add cars, porticos, stairs, or gates if they are not explicitly present in the 2D plan. Return ONLY the generated text prompt.",
