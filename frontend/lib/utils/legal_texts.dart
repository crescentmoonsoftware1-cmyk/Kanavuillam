class LegalTexts {
  static const String termsAndConditions = '''
# Terms and Conditions for Kanav illam

Last Updated: June 2026

Welcome to Kanav illam. By downloading, accessing, or using our mobile application, you agree to be bound by these Terms and Conditions.

## 1. Acceptance of Terms
By accessing or using Kanav illam, you acknowledge that you have read, understood, and agree to comply with these Terms and Conditions.

## 2. Description of Services
Kanav illam provides:
* AI-powered floor plan recognition and analysis.
* 3D architectural visualization and virtual walkthroughs.
* Vastu Shastra compliance analysis.
* Construction quantity and cost estimation.
* PDF report generation and export services.

The information provided is intended for informational and planning purposes only and should not replace professional architectural, engineering, legal, or construction advice.

## 3. User Accounts
Users may be required to create an account to access certain features.
You agree to:
* Provide accurate information.
* Maintain the confidentiality of your login credentials.
* Accept responsibility for all activities occurring under your account.

## 4. User Content
Users retain ownership of uploaded images, sketches, floor plans, and documents.
By uploading content, you grant Kanav illam a limited license to process, analyze, store, and generate reports necessary for providing app services.
You agree not to upload:
* Illegal content.
* Copyright-infringing material.
* Harmful, malicious, or offensive content.

## 5. AI-Generated Outputs
AI-generated plans, reports, Vastu recommendations, visualizations, and cost estimates are provided on an "as-is" basis.
We do not guarantee:
* Engineering accuracy.
* Structural safety.
* Construction feasibility.
* Legal compliance.
* Exact material or labor costs.
Users should consult qualified professionals before making construction or investment decisions.

## 6. Payments and Premium Services
Certain features may require payment through approved payment providers including Razorpay or Stripe.
Payments are generally non-refundable except where required by applicable law.

## 7. Intellectual Property
All application content, branding, software, algorithms, UI designs, and generated system components remain the property of Kanav illam unless otherwise stated.
Unauthorized copying, modification, or distribution is prohibited.

## 8. Limitation of Liability
Kanav illam shall not be liable for:
* Construction losses.
* Design defects.
* Cost overruns.
* Property damages.
* Business interruptions.
* Indirect or consequential damages arising from app usage.

## 9. Service Availability
We reserve the right to modify, suspend, or discontinue any feature at any time without prior notice.

## 10. Termination
We may suspend or terminate accounts that violate these Terms or misuse the platform.

## 11. Governing Law
These Terms shall be governed by and interpreted in accordance with the laws of India.

## 12. Contact Information
For support or legal inquiries:
Email: support@kanavillam.com

By using Kanav illam, you agree to these Terms and Conditions.
''';

  static const String privacyPolicy = '''
# Privacy Policy for Kanav illam

Last Updated: June 2026

Kanav illam values your privacy. This Privacy Policy explains how we collect, use, store, and protect your information.

## 1. Information We Collect

    Personal Information
* Name
* Email address
* User account details
* Profile information

    Uploaded Content
* Floor plan images
* Hand-drawn sketches
* Property dimensions
* Generated reports

    Device Information
* Device type
* Operating system
* App version
* Crash logs
* Analytics information

    Payment Information
Payments are processed securely through third-party payment providers such as Razorpay and Stripe. We do not store complete card or banking information.

## 2. How We Use Information
We use collected information to:
* Create and manage accounts.
* Generate AI-powered architectural analysis.
* Produce Vastu compliance reports.
* Calculate construction estimates.
* Generate downloadable PDF reports.
* Improve application performance.
* Provide customer support.
* Prevent fraud and misuse.

## 3. AI Processing Services
Uploaded images and design information may be securely transmitted to:
* Google Gemini AI services.
* OpenRouter AI services.
* Internal processing servers.
These services are used solely to provide requested AI features.

## 4. Data Storage
User data may be stored using Supabase cloud infrastructure and associated storage systems.
Reasonable security measures are implemented to protect stored information.

## 5. Data Sharing
We do not sell personal information.
Information may be shared with:
* Cloud infrastructure providers.
* AI service providers.
* Payment processors.
* Legal authorities when required by law.

## 6. Data Retention
We retain information only as long as necessary to:
* Provide services.
* Maintain user accounts.
* Meet legal obligations.
* Resolve disputes.
Users may request account deletion at any time.

## 7. Children's Privacy
This application is not intended for children under 13 years of age.
We do not knowingly collect information from children.

## 8. Security
We implement industry-standard security measures to protect user information.
However, no online system can guarantee absolute security.

## 9. User Rights
Users may:
* Access their information.
* Correct inaccurate information.
* Request deletion of their data.
* Withdraw consent where applicable.
Requests can be submitted through our support email.

## 10. Third-Party Services
The application may use third-party services including:
* Supabase
* Google Gemini
* OpenRouter
* Stripe
* Razorpay
Their respective privacy policies may also apply.

## 11. Changes to This Policy
We may update this Privacy Policy periodically.
Updated versions will be posted within the application.

## 12. Contact Us
For privacy-related inquiries:
Email: support@kanavillam.com

By using Kanav illam, you consent to this Privacy Policy.
''';
}
